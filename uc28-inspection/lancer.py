...
class App(tk.Tk):
    ...
    def _build_header(self):
        ...
        tk.Label(
            header, text="Bureau Veritas Certification · Démonstrateur IA · Hackathon Capgemini × Anthropic 2026",
    ...
    def _build_status_row(self):
        ...
        self._backend_dot  = self._make_indicator(row, "Backend FastAPI", "localhost:8000", 0)
        self._frontend_dot = self._make_indicator(row, "Frontend Vite",   "localhost:5173", 1)
    ...
    def _make_indicator(self, parent, title, url, col):
        ...
        dot = tk.Label(frame, text="●", font=("Segoe UI", 14),
                       fg=C_MUTE, bg=C_PANEL)
        dot.grid(row=0, column=0, rowspan=2, padx=(0, 8))
        tk.Label(frame, text=title, font=("Segoe UI", 10, "bold"),
                 fg=C_TEXT, bg=C_PANEL, anchor="w").grid(row=0, column=1, sticky="w")
        tk.Label(frame, text=url, font=("Consolas", 9),
                 fg=C_MUTE, bg=C_PANEL, anchor="w").grid(row=1, column=1, sticky="w")
        return dot
    ...
    def _build_footer(self):
        ...
        self._btn_start = tk.Button(
            footer, text="▶  Démarrer",
            font=("Segoe UI", 10, "bold"),
            bg=C_ACCENT, fg="white",
            activebackground=C_ACCENT_DIM, activeforeground="white",
            relief="flat", padx=16, pady=8, cursor="hand2",
            command=self._start_services, state="disabled",
        )
        self._btn_start.grid(row=0, column=0, padx=(0, 8))

        self._btn_browser = tk.Button(
            footer, text="🌐  Ouvrir l'application",
            font=("Segoe UI", 10),
    ...
    def _set_dot(self, dot: tk.Label, color: str):
        self.after(0, lambda: dot.config(fg=color))
    ...
    def _setup_worker(self):
        ...
        if not example.exists():
            example = ROOT.parent / ".env.example"
        if not env_file.exists():
            if example.exists():
                import shutil
                shutil.copy(example, env_file)
                self._log("⚠  .env créé depuis .env.example — renseignez ANTHROPIC_API_KEY", "warn")
            else:
                self._log("⚠  backend/.env absent — créez-le avec ANTHROPIC_API_KEY=sk-...", "warn")
        else:
        ...
        if not VENV_PYTHON.exists():
            self._log("→  Création du venv Python…", "info")
            r = subprocess.run(
            ...
            if r.returncode != 0:
                ...
                self._log("→  Installation des dépendances Python (peut prendre 2-3 min)…", "warn")
                r2 = subprocess.run(
        ...
        # npm
        if not (FRONTEND / "node_modules" / ".bin" / "vite").exists() and \
           not (FRONTEND / "node_modules" / ".bin" / "vite.cmd").exists():
            self._log("→  Installation npm (première fois, ~1 min)…", "warn")
            r = subprocess.run(
    ...
    def _start_worker(self):
        ...
        try:
            ...
            _procs.append(p_back)
            threading.Thread(target=self._tail,
                             args=(p_back, "backend"), daemon=True).start()
            self._log(f"✓  Backend démarré (pid {p_back.pid})", "ok")
            self._set_dot(self._backend_dot, C_ACCENT)
        except Exception as e:
            self._log(f"✗  Backend : {e}", "err")
            self._set_dot(self._backend_dot, C_DANGER)
            self.after(0, lambda: self._btn_start.config(state="normal"))
            return
        ...
        try:
            ...
            _procs.append(p_front)
            threading.Thread(target=self._tail,
                             args=(p_front, "frontend"), daemon=True).start()
            self._log(f"✓  Frontend démarré (pid {p_front.pid})", "ok")
            self._set_dot(self._frontend_dot, C_ACCENT)
        except Exception as e:
            self._log(f"✗  Frontend : {e}", "err")
            self._set_dot(self._frontend_dot, C_DANGER)
            self.after(0, lambda: self._btn_start.config(state="normal"))
            return

        self._log("\n✓  Application disponible → http://localhost:5173", "ok")
    ...
    def _stop_services(self):
        ...
        _kill_all()
        self._set_dot(self._backend_dot,  C_MUTE)
        self._set_dot(self._frontend_dot, C_MUTE)
