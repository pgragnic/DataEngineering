...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col bg-canvas">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col" style={{ backgroundImage: "linear-gradient(to bottom, rgba(0,112,173,0.04) 0px, transparent 120px)" }}>
        {view === "login" && (
          <LoginScreen
            onLogin={(userData) => {
              setUser(userData)
        ...
        {view === "portail" && (
          <SupplierPortal theme={theme} onBack={() => setView("login")} externalDocs={savedReports} lang={lang} />
        )}
        {view === "clients" && (
          <ClientList onSelectClient={(client) => { setSelectedClient(client); setView("dashboard") }} theme={theme} lang={lang} />
        )}
