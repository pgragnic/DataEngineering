...
const ST = { "TERMINÉ": "common.status_termine", "PROCHAIN": "common.status_prochain", "PLANIFIÉ": "common.status_planifie" }

export default function MapCard({ audits, theme, compact = false, focusCoords = null, transport = "voiture", showRoute = true, lang = "FR" }) {
  const t = useT(lang)
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      >
        ...
        <Marker position={[AUDITEUR.coords.lat, AUDITEUR.coords.lng]} icon={makeStartIcon(brandColor)}>
          <Popup>
            ...
            <div className="text-[11px] text-gray-500">{AUDITEUR.localisation} — {t("common.origin")}</div>
          </Popup>
        </Marker>
          ...
          >
            <Popup>
              ...
              <div className="text-[11px] mt-0.5" style={{ color: colors[audit.statut] }}>
                {t(ST[audit.statut]) || audit.statut}
              </div>
            </Popup>
          </Marker>
        ))}
      ...
      {/* Bouton tout voir */}
      {compact && (
        <div className="px-3 py-1.5 flex justify-end border-t border-gray-100 bg-gray-50">
          <button
            onClick={() => mapRef.current?.flyToBounds(L.latLngBounds(straightPoints), { padding: [30, 30], duration: 1 })}
            className="text-[10px] text-gray-400 hover:text-gray-700 border border-gray-200 rounded px-2 py-0.5 bg-white hover:bg-gray-100 transition-colors"
