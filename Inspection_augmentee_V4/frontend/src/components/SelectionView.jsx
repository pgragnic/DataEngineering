...
const TRANSPORT_META_BASE = {
  voiture: { Icon: Car,   bg: "bg-blue-100",   text: "text-blue-600" },
  TC:      { Icon: Train, bg: "bg-purple-100", text: "text-purple-600" },
  velo:    { Icon: Bike,  bg: "bg-green-100",  text: "text-green-600" },
}
...
function genererPlanning(audits, transport, config) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (!pauseInserted && cursor >= pauseMin && cursor < pauseEnd) {
      result.push({ id: "__pause__", isPause: true, heureMin: cursor, dureeMin: pauseDuree, heure: minToHeure(cursor), titre: "lunch" })
      cursor += pauseDuree
      pauseInserted = true
...
export default function SelectionView({ selectedIds, transport: initTransport, config: initConfig, onDemarrer, onBack, theme, lang }) {
  const t = useT(lang)
  ...
  const TRANSPORT_META = {
    voiture: { ...TRANSPORT_META_BASE.voiture, label: t("planning.car") },
    TC:      { ...TRANSPORT_META_BASE.TC,      label: t("planning.tc") },
    velo:    { ...TRANSPORT_META_BASE.velo,    label: t("planning.bike") },
  }
      ...
      <div className="page-bar">
        ...
        <span className="text-xs font-semibold text-gray-700">
          {selectedAudits.length === 1
            ? t("selection.header_title_singular", { n: 1 })
            : t("selection.header_title_plural", { n: selectedAudits.length })}
        </span>
        <div className="w-36" />
      </div>
      ...
      <div className="page-content">
        {selectedAudits.length === 0 ? (
          <div className="text-center text-gray-400 text-sm py-16">
            {t("selection.no_selection_message")}
            <br />
            <button onClick={onBack} className={`mt-3 text-xs underline ${ag ? "text-[#1AAED2]" : ar ? "text-[#4B87F8]" : "text-blue-600"}`}>
              {t("selection.back_to_dashboard")}
            </button>
          </div>
        ) : (
          <div className="grid-12">
                    ...
                    return (
                      <div key="pause" className="flex items-center gap-3 py-2 px-4">
                        ...
                        <div className="flex-1 bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-xs font-medium text-gray-600">{t("selection.lunch")}</div>
                    ...
                    <div key={item.id}>
                      ...
                      <div className={`rounded-xl border p-4 flex items-start gap-4 shadow-sm transition-opacity ${
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.hors_plage && <div className="text-[10px] text-red-600 font-medium mt-0.5">{t("selection.over_day_end")}</div>}
                        </div>
                        {displayStatut === "TERMINÉ" ? (
                          <span className="text-[10px] font-semibold text-green-600 shrink-0 px-2 py-1.5">{t("common.status_done")}</span>
                        ) : (
                          <button
            ...
            <div className="col-span-4">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{t("selection.selected_route")}</h2>
              <MapCard audits={auditsForMap} theme={theme} compact transport={transportActif} lang={lang} />
            </div>
          </div>
        )}
      </div>
